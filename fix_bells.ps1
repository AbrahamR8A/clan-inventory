$viewsDir = "c:\Users\AbrahamR8A\Documents\Proyecto CLAN_INVENTORY\clan-inventory\src\main\webapp"
...
# Regex para atrapar TODO el bloque de la campana, incluyendo comentarios duplicados
$pattern = '(?si)(<!--\s*Nav Item - Alerts\s*-->\s*)*<li\s+class="nav-item\s+dropdown\s+no-arrow\s+mx-1"\s*>.*?</li>\s*(?=<div\s+class="topbar-divider)'
...
Get-ChildItem -Path $viewsDir -Filter "*.jsp" -Recurse | ForEach-Object {
    ...
    # Reemplazamos si encontramos el bloque
...
Write-Host "Total arreglados: $count"

...
public class SolicitudesDepositoDao extends BaseDao {
    ...
    // ─── 5. Obtener Cabecera de Solicitud por ID ──────────────────────────
    ...
    // ─── 6. Marcar como Entregada y Generar Movimiento ──────────────────────
    ...
    public boolean marcarComoEntregada(int idSolicitud, int idEncargado) {
        ...
        try {
            ...
            // Apagamos el autocommit para asegurar que toda la transacción se ejecute en bloque
            conn.setAutoCommit(false);

            // 1. Actualizamos el estado de la solicitud y registramos la fecha exacta de entrega
            ...
            // 2. Obtenemos qué productos y cantidades conformaban esta solicitud
            ...
            // 3. Registramos cada producto en la tabla movimientos
           String sqlInsertMovimiento = "INSERT INTO movimientos (tipo, fecha_movimiento, cantidad, id_productos, id_responsable, id_solicitudes) " +
                    "VALUES (?, CURRENT_TIMESTAMP(), ?, ?, ?, ?)";

            try (PreparedStatement pstmt3 = conn.prepareStatement(sqlInsertMovimiento)) {
                for (Detalles d : detallesEntrega) {
                    pstmt3.setString(1, "salida");
                    pstmt3.setInt(2, d.getCantidad());
                    pstmt3.setInt(3, d.getProducto().getIdProductos());
                    pstmt3.setInt(4, idEncargado); // El encargado que está registrando en el sistema
                    pstmt3.setInt(5, idSolicitud); // Para tener trazabilidad de qué solicitud generó este movimiento
            ...
            // Si algo falla, revertimos todo para que no queden datos corruptos
            if (conn != null) {
                try {
                    conn.rollback();
    ...
    // ─── 7. Listar Historial de Entregas ─────────────────────────────────────────
